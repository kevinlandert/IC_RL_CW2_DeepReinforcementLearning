...
class Environment:
    ...
    def show(self, agent_state,draw_greedy, agent,new,random_seed):
        ...
        #cv2.imwrite('environment.png', self.image)
                # Draw the state cell borders
            
            

        #Draw the greedy policyline
        
        if draw_greedy == 'start': 
            ...
            if new: 
                ...
                for i in range(100):
                    ...
                    q = agent.dqn.get_q_value(state)
                    dirs = np.argmax(q)
                    action = agent.directions[dirs]
                    next_state,_ = self.step(state,action)
            
                    #Transform to grid location
                    old  = (int(state[0] * self.magnification),int((1 - state[1]) *self.magnification))
                    new = (int(next_state[0] * self.magnification),int((1 - next_state[1]) *self.magnification))
                    self.olds.append(old)
                    self.news.append(new)
                    #cv2.line(self.image, old, new, (200, 50, 50), thickness=4)
                    state = next_state
                
                
            
            for i in range(len(self.olds)):
                
                cv2.line(self.image, self.olds[i], self.news[i], (200, 50, 50), thickness=4)
                
        elif draw_greedy == 'full':
            
            state = self.init_state
        
            for i in range(min(agent.step,100)):
        ...
        nr = random_seed % 100
        cv2.imwrite('environment_' + str(nr)+ '.png', self.image)
        cv2.waitKey(3)
        
        
        
    def draw_greedy(self, agent_state,agent,success):
        # Create the background / obstacle
        window_top_left = (0, 0)
        window_bottom_right = (self.magnification * 1, self.magnification * 1)
        cv2.rectangle(self.image, window_top_left, window_bottom_right, (50, 50, 50), thickness=cv2.FILLED)
        ...
        #info = str(agent.plot_nr) + success + "_" + str(agent.num_steps_taken) + "_" + str(agent.episode_length)
        info = success
    
        
        cv2.imwrite('results/greedy_policy{}.png'.format(info), self.image)
        #cv2.imshow("Environment", self.image)
        cv2.waitKey(1)
